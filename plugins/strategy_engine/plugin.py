...
def _render_market_receipt(meta: dict, symbol: str) -> None:
    """
    Display a compact data receipt so the user can visually verify
    they are analyzing the right asset, time period, and data source
    before reading the signals below.

    Shows inferred fields always; shows richer fields (interval, source,
    asset class) only when market_data provides them — no placeholders
    for missing data, which would just add visual noise.
    """
    st.subheader("📊 Market Data")

    # ---- Row 1: "What data am I looking at?" ────────────────────────
    # Symbol, Interval, Period, Candles — the four identity fields.
    # Interval and Period are now always present (market_data provides them).
    period_label = "Period (est.)" if meta.get("period_estimated") else "Period"
    r1 = st.columns(4)
    r1[0].metric("Symbol",   meta.get("symbol", symbol))
    r1[1].metric("Interval", meta.get("interval", "—"))
    r1[2].metric(period_label, meta.get("period_str", "—"),
                 help="Exact period string or date range from market_data. "
                      "Shown as '(est.)' only when market_data provides no "
                      "period info and the value is inferred from candle count.")
    r1[3].metric("Candles",  meta.get("candle_count", "?"))

    # ---- Row 2: "Does this data look right?" ────────────────────────
    # Last price, price range, and available columns.
    row2: list[tuple[str, str]] = []

    if "price_last" in meta:
        row2.append(("Last Price",
                     f"{meta['price_last']:,.4f}"))
    if "price_min" in meta and "price_max" in meta:
        row2.append(("Price Range",
                     f"{meta['price_min']:,.4f} – {meta['price_max']:,.4f}"))
    if meta.get("columns"):
        row2.append(("Columns", "  ·  ".join(meta["columns"])))

    # ---- Row 3: optional provenance fields ──────────────────────────
    # Only shown when market_data supplies them; no placeholders.
    row3: list[tuple[str, str]] = []
    for key, label in [("source", "Source"), ("asset_class", "Asset Class"),
                       ("currency", "Currency")]:
        if key in meta:
            row3.append((label, meta[key]))

    for row in (row2, row3):
        if row:
            rcols = st.columns(min(len(row), 4))
            for i, (label, value) in enumerate(row):
                rcols[i].metric(label, value)

    st.markdown("---")

def _sync_from_market_data(
    context,
    interval_options: list[str],
    period_options:   list[str],
) -> None:
    """
    Pull interval and period from market_data's current settings via the
    market.settings capability and update this plugin's session_state.

    Calls st.rerun() on success so the dropdowns reflect the new values
    immediately. On failure (capability not available, unknown values, etc.)
    shows a warning without touching the current selections.
    """
    try:
        response = context.request("market.settings", {})
    except Exception as e:
        st.warning(
            f"⚠️ Could not reach market.settings: {e}  "
            f"— is market_data upgraded to v1.2+?"
        )
        return

    if response.get("status") != "success":
        st.warning(
            f"⚠️ market.settings returned an error: "
    ...
    new_symbol = settings.get("symbol")
    if new_symbol:
        if st.session_state.get("se_symbol") != new_symbol:
            st.session_state["se_symbol"] = new_symbol
            changed.append(f"symbol → {new_symbol}")

    new_interval = settings.get("interval")
    if new_interval and new_interval in interval_options:
        if st.session_state.get("se_interval") != new_interval:
            st.session_state["se_interval"] = new_interval
            changed.append(f"interval → {new_interval}")
    elif new_interval:
        st.warning(
            f"⚠️ market_data interval '{new_interval}' is not in this "
            f"plugin's interval list — ignoring."
        )

    new_period = settings.get("period")
    if new_period and new_period in period_options:
        if st.session_state.get("se_period") != new_period:
            st.session_state["se_period"] = new_period
            changed.append(f"period → {new_period}")
    elif new_period:
        st.warning(
            f"⚠️ market_data period '{new_period}' is not in this "
            f"plugin's period list — ignoring."
        )

    if changed:
        st.rerun()
    else:
        st.info("✓ Already in sync with market_data.")
...
def render_ui(context) -> None:
    st.title("📈 Strategy Engine")
    st.caption("NiteTrader · Multi-Strategy Signal Dashboard")

    # ----------------------------------------------------------------
    # Strategy library panel
    # ----------------------------------------------------------------
    n_ext = len(_external_names)
    n_err = len(_discovery_errors)
    issue_label = f"⚠️ {n_err} issue(s)" if n_err else "✅ clean"

    # Show discovery errors visibly OUTSIDE the expander.
    # Errors buried only inside a collapsed expander are effectively invisible.
    if _discovery_errors:
        for _fname, _msg in _discovery_errors.items():
            if _msg.startswith("Note:"):
                st.info(f"💡 Strategy library: `{_fname}` — {_msg}", icon=None)
            else:
                st.warning(f"⚠️ Strategy file `{_fname}` not loaded: {_msg}")

    with st.expander(
        f"📂 Strategy Library  "
        f"({len(BUILTIN_STRATEGIES)} built-in · {n_ext} external · {issue_label})",
        expanded=(n_err > 0),   # auto-open when there are issues
    ):
        ...
        with col_btn:
            if st.button("🔄 Reload Strategies"):
                ...
                st.rerun()
        with col_path:
            st.caption(f"`{STRATEGIES_DIR}`")

        rows = []
        for name in STRATEGIES:
            m      = _strategy_meta.get(name, {})
            source = m.get("source", "built-in")
            ffile  = m.get("file", "—")
            rows.append({
                "Strategy": name,
                "Type":     m.get("type", "—"),
                "Source":   source,
                "File":     ffile if source != "built-in" else "built-in",
            })
        st.table(pd.DataFrame(rows).set_index("Strategy"))

        if _discovery_errors:
            st.markdown("**Discovery issues:**")
            for fname, msg in _discovery_errors.items():
                icon = "💡" if msg.startswith("Note:") else "⚠️"
                st.caption(f"{icon} `{fname}`: {msg}")

    # ----------------------------------------------------------------
    # Data request parameters
    # ----------------------------------------------------------------
    INTERVAL_OPTIONS = ["1m","2m","5m","15m","30m","60m","90m","1h",
                        "1d","1wk","1mo"]
    PERIOD_OPTIONS   = ["1d","5d","1mo","3mo","6mo","1y","2y","5y","10y","ytd","max"]

    col_sym, col_int, col_per, col_sync = st.columns([2, 1, 1, 0.8])
    with col_sym:
        symbol = st.text_input(
            "Asset Symbol",
            value=st.session_state.get("se_symbol", "BTC-USD"),
            # No key= — managed manually so sync can write without ownership conflict
        )
        st.session_state["se_symbol"] = symbol
    with col_int:
        # No key= — we manage "se_interval" in session_state ourselves so that
        # _sync_from_market_data() can write to it without hitting the
        # "cannot modify after widget instantiated" error.
        _int_val = st.session_state.get("se_interval", "1d")
        interval = st.selectbox(
            "Interval",
            INTERVAL_OPTIONS,
            index=INTERVAL_OPTIONS.index(_int_val)
                  if _int_val in INTERVAL_OPTIONS else 0,
        )
        st.session_state["se_interval"] = interval   # keep state current

    with col_per:
        _per_val = st.session_state.get("se_period", "6mo")
        period = st.selectbox(
            "Period",
            PERIOD_OPTIONS,
            index=PERIOD_OPTIONS.index(_per_val)
                  if _per_val in PERIOD_OPTIONS else 5,
        )
        st.session_state["se_period"] = period        # keep state current
    with col_sync:
        st.markdown("&nbsp;", unsafe_allow_html=True)   # vertical alignment nudge
        if st.button("🔄 Sync", help="Pull interval and period from market_data's current settings"):
            _sync_from_market_data(context, INTERVAL_OPTIONS, PERIOD_OPTIONS)

    st.markdown("---")
    st.subheader("Strategy Selection & Weights")
    st.caption(
        "Check strategies to include. "
        "Click **ℹ** for a description of each strategy. "
        "Adjust the weight slider to control how much each strategy influences the combined signal."
    )

    # Initialise / refresh session state
    if "se_selected" not in st.session_state:
        st.session_state["se_selected"] = {}
    if "se_weights" not in st.session_state:
        st.session_state["se_weights"] = {}

    for name in STRATEGIES:
        st.session_state["se_selected"].setdefault(name, True)
        st.session_state["se_weights"].setdefault(name, DEFAULT_WEIGHT)

    # Remove stale keys for unloaded strategies
    for n in [k for k in list(st.session_state["se_selected"]) if k not in STRATEGIES]:
        del st.session_state["se_selected"][n]
        del st.session_state["se_weights"][n]

    # ---- Per-strategy row: [checkbox] [ℹ popover] [weight slider] ----
    for name in STRATEGIES:
        meta        = _strategy_meta.get(name, {})
        description = meta.get("description", "")
        stype       = meta.get("type", "")
        ...
        col_chk, col_info, col_slider = st.columns([1.8, 0.15, 2.2])

        with col_chk:
            enabled = st.checkbox(
                f"{name}{badge}",
                value=st.session_state["se_selected"].get(name, True),
                key=f"se_chk_{name}",
            )
            st.session_state["se_selected"][name] = enabled

        with col_info:
            with st.popover("ℹ", help=f"About: {name}"):
                st.markdown(f"**{name}**")
                if stype:
                    st.caption(f"*{stype}*")
                if description:
                ...
                # Show indicator list for declarative strategies
                inds = meta.get("indicators")
                if inds:
                    st.caption(f"Indicators: {', '.join(inds)}")
                ffile = meta.get("file")
                if ffile and meta.get("source", "built-in") != "built-in":
                    st.caption(f"File: `{ffile}`")
        ...
        with col_slider:
            if enabled:
                w = st.slider(
                    ...
                    min_value=0.1,
                    max_value=3.0,
                    value=st.session_state["se_weights"].get(name, DEFAULT_WEIGHT),
                    step=0.1,
                    key=f"se_wt_{name}",
                    label_visibility="collapsed",
                )
                st.session_state["se_weights"][name] = w
            else:
                st.caption("—")

    selected_names = [n for n in STRATEGIES if st.session_state["se_selected"].get(n)]
    active_weights = {n: st.session_state["se_weights"].get(n, DEFAULT_WEIGHT)
                      for n in selected_names}

    if not selected_names:
        st.warning("⚠️ Enable at least one strategy to run analysis.")
        return

    st.markdown("---")

    if st.button("Run Analysis", type="primary", use_container_width=True):
        with st.spinner(f"Fetching {symbol} · running {len(selected_names)} strategies…"):
            response = handle_request(
                {
                    "capability": "market.strategy.signal",
                    "payload": {
                        "symbol":     symbol,
                        "interval":   interval,
                        "period":     period,
                        "strategies": selected_names,
        ...
        errors       = data.get("strategy_errors", {})
        candle_count = data.get("candle_count", "?")
        market_meta  = data.get("market_meta", {})

        # ---- Market data receipt ----------------------------------------
        # Shown before signals so the user can verify they're looking at
        # the right asset, time period, and data source before trusting results.
        _render_market_receipt(market_meta, symbol)
        ...
        style = SIGNAL_STYLE.get(sig, {"emoji": "⚪", "color": "#888"})

        st.markdown(
            f"<h2 style='color:{style['color']}'>{style['emoji']} {sig}</h2>",
            unsafe_allow_html=True,
        )
        col_s, col_c = st.columns(2)
        col_s.metric("Weighted Score", f"{score:+.3f}",
                     help="−1.0 = full SELL  ·  +1.0 = full BUY")
        col_c.metric("Confidence", f"{conf:.0f}%",
                     help="Distance from neutral (0 = split, 100 = unanimous)")
        ...
        for name in selected_names:
            s   = signals.get(name, "HOLD")
            err = errors.get(name)
            rows.append({
                "Strategy": name + (" ⚠️" if err else ""),
                "Signal":   _signal_label(s),
                "Weight":   f"{active_weights.get(name, DEFAULT_WEIGHT):.1f}",
                "Vote":     f"{SIGNAL_SCORES.get(s, 0.0):+.1f}",
                "Weighted": f"{SIGNAL_SCORES.get(s, 0.0) * active_weights.get(name, DEFAULT_WEIGHT):+.2f}",
            })
        st.table(pd.DataFrame(rows).set_index("Strategy"))

        if errors:
            with st.expander("⚠️ Strategy warnings"):
                for name, err in errors.items():
                    st.warning(f"**{name}**: {err}  (defaulted to HOLD)")

        st.caption(
            f"Strategies run: {len(selected_names)} · "
